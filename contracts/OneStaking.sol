...
contract OneStaking is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accFuzzPerShare; // Accumulated CAKEs per share, times 1e12. See below.
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accFuzzPerShare = accFuzzPerShare.add(cakeReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cakeReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accFuzzPerShare = pool.accFuzzPerShare.add(cakeReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit() public payable {
        ...
        if(msg.value > 0) {
            IWONE(WONE).deposit{value: msg.value}();
            assert(IWONE(WONE).transfer(address(this), msg.value));
    ...
    function withdraw(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            IWONE(WONE).withdraw(_amount);
            safeTransferONE(address(msg.sender), _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accFuzzPerShare).div(1e12);
