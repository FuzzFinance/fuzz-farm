...
import '@fuzzfinance/fuzz-swap-lib/contracts/math/SafeMath.sol';
...
interface IMigratorChef {
    // Perform LP token migration from legacy FuzzFinance to FuzzSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to FuzzFinance LP tokens.
    // FuzzSwap must mint EXACTLY the same amount of FuzzSwap LP tokens or
    // else something bad will happen. Traditional FuzzFinance does not
    // do that so be careful!
    function migrate(IHRC20 token) external returns (IHRC20);
}

// MasterChef is the master of Fuzz. He can make Fuzz and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        fuzz.mint(address(syrup), fuzzReward);
    ...
    // Safe fuzz transfer function, just in case if rounding error causes pool to not have enough CAKEs.
    function safeFuzzTransfer(address _to, uint256 _amount) internal {
        syrup.safeFuzzTransfer(_to, _amount);
    }
