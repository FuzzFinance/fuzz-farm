...
contract('MasterChef', ([alice, bob, carol, dev, minter]) => {
  ...
  it('real case', async () => {
    ...
    assert.equal(
      (await this.fuzz.balanceOf(this.lottery.address)).toString(),
      '0'
    );
    ...
    assert.equal((await this.fuzz.balanceOf(carol)).toString(), '5');
  });
  ...
  it('setReceiver', async () => {
    ...
    assert.equal((await this.fuzz.balanceOf(carol)).toString(), '7');
    await this.lottery.setReceiver(alice, { from: dev });
    assert.equal((await this.lottery.pendingReward('1')).toString(), '7');
    await this.lottery.harvest(1, { from: dev });
    assert.equal((await this.fuzz.balanceOf(alice)).toString(), '15');
  });
