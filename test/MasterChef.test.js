...
contract('MasterChef', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        ...
        this.chef = await MasterChef.new(this.fuzz.address, this.syrup.address, dev, '1000', '100', { from: minter });
        await this.fuzz.transferOwnership(this.chef.address, { from: minter });
    ...
    it('real case', async () => {
      ...
      assert.equal((await this.fuzz.balanceOf(alice)).toString(), '0');
      await this.chef.deposit(1, '20', { from: alice });
      await this.chef.withdraw(1, '20', { from: alice });
      assert.equal((await this.fuzz.balanceOf(alice)).toString(), '263');

      await this.fuzz.approve(this.chef.address, '1000', { from: alice });
      await this.chef.enterStaking('20', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      await this.chef.enterStaking('0', { from: alice });
      assert.equal((await this.fuzz.balanceOf(alice)).toString(), '993');
    ...
    it('deposit/withdraw', async () => {
      ...
      assert.equal((await this.fuzz.balanceOf(alice)).toString(), '999');
      assert.equal((await this.fuzz.balanceOf(dev)).toString(), '100');
      ...
      assert.equal((await this.fuzz.balanceOf(bob)).toString(), '125');
    ...
    it('updaate multiplier', async () => {
      ...
      await this.fuzz.approve(this.chef.address, '100', { from: alice });
      await this.fuzz.approve(this.chef.address, '100', { from: bob });
      ...
      assert.equal((await this.fuzz.balanceOf(alice)).toString(), '700');
      assert.equal((await this.fuzz.balanceOf(bob)).toString(), '150');
